...
from certify.smoothing import Smooth2
...
def optimizing_noise(
    ...
    model: str,
    distribution = "norm" # default is norm distribution
    ):
    ...
    noise_norm = noise_generator.sample_norm(nt_shape,distribution).repeat(1, batch_size_iteration).view(-1, noise_generator.d)
    ...
    torch.save(noise_generator.distribution_transformer, os.path.join(save_dir, "checkpoint-distribution-transformer{}{}{}".format(model,distribution,pred_train)))
    ...
    classifier.eval()
    smoothed_classifier = Smooth2(classifier, d, num_classes, noise_generator,noise_generator, classifier.device,distribution=distribution)
    ...
    while (iter_scale < max_iter_scale) and (decay_scale < max_decay_scale):
        ...
        for (X,) in data_loader:
            ...
            cA, robust_radius,_  = smoothed_classifier.bars_certify(X, n0, n, t, alpha, batch_size_memory)
        ...
        for (X,) in data_loader:
            ...
            cA, robust_radius,_ = smoothed_classifier.bars_certify(X, n0, n, t + ptb_t_scale, alpha, batch_size_memory)
    ...
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    w = open(os.path.join(save_dir, "t{}{}{}".format(model,distribution,pred_train)), "w")
    w.write("%.6e" % (t))
    w.close()
...
def optimizing_sigma(
    x_train: torch.tensor, # input data for optimization
    classifier: nn.Module, # classifier model
    sigma_generator: nn.Module, # noise generator model
    criterion_shape: nn.Module, # loss function for optimizing noise shape
    learning_rate_shape: float, # learning rate for optimizing noise shape
    nt_shape: int, # number of noised samples for optimizing noise shape
    num_epochs_shape: int, # epochs for optimizing noise shape
    ...
    assert classifier.device == sigma_generator.device
    ...
    opt = optim.Adam(sigma_generator.sigma.parameters(), lr=learning_rate_shape)

    noise_norm = sigma_generator.sample_norm(nt_shape).repeat(1, batch_size_iteration).view(-1, sigma_generator.d)
    loss_record = AverageMeter()
    classifier.train()
    for epoch in range(0, num_epochs_shape + 1):
        for i, (X,) in zip(range(1, len(data_loader) + 1), data_loader):
            ...
            if X.size(0) < batch_size_iteration:
                ...
                noise_idx = torch.arange(noise_norm.size(0)).to(X.device)
            noise_feat = sigma_generator.norm_to_feat(noise_norm[noise_idx, :])
            score = classifier.score(X.repeat((nt_shape, 1)).to(X.device), noise_feat)
            loss = criterion_shape(score, sigma_generator.get_weight())
    ...
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    torch.save(sigma_generator.sigma, os.path.join(save_dir, "checkpoint-sigma"))